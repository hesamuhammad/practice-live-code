...
function hitungKembalian(bayar, harga) {
  ...
  for (let i = 0; i < ketersediaanUang.length; i++) {
    while (kembalian >= ketersediaanUang[i]['nominal'] && ketersediaanUang[i]['jumlah'] > 0) {
      if (ketersediaanUang[i]['jumlah'] > 0) {
        if (!obj[ketersediaanUang[i]['nominal']]) {
          obj[ketersediaanUang[i]['nominal']] = 1
        } else {
          obj[ketersediaanUang[i]['nominal']] += 1
        }
        kembalian -= ketersediaanUang[i]['nominal']
        ketersediaanUang[i]['jumlah'] -= 1
      }
    }
  }
  return obj
